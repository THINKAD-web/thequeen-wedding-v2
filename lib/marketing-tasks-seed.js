...
const { REPLACEMENT_TASKS, NEW_DRESSSHOW_TASKS, REPLACE_KEYS } = require("./marketing-dressshow-tasks");

/** 8-2·8-3 기존 queen 태스크 sort_order 재배치 (드레스쇼 피드 3건 삽입) */
const AUG_SORT_BUMP = {
  "q82-2": 2,
  "q82-3": 4,
  "q83-2": 2,
...
const SEED_TASKS = [
  ...BASE_SEED_TASKS.filter((t) => !REPLACE_KEYS.has(t.task_key)).map((t) =>
    AUG_SORT_BUMP[t.task_key] !== undefined ? { ...t, sort_order: AUG_SORT_BUMP[t.task_key] } : t
  ),
  ...REPLACEMENT_TASKS,
  ...NEW_DRESSSHOW_TASKS,
];
