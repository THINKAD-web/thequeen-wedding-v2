...
async function seedTasks(pool, { force = false } = {}) {
  if (force) {
    // 8월만 교체 — 7월 완료 상태 유지
    await pool.query("DELETE FROM tasks WHERE week_id >= 5");
  }
  ...
  for (const t of SEED_TASKS) {
    const r = await pool.query(
      ...
      ON CONFLICT (task_key) DO UPDATE SET
        ...
        tags = EXCLUDED.tags,
        tips = EXCLUDED.tips,
        status = CASE WHEN EXCLUDED.week_id >= 5 THEN 'pending' ELSE tasks.status END,
        updated_at = NOW()
      RETURNING id`,
      [
        t.task_key,
...
/** 브랜드별 디자인 토큰 (admin UI · 미리보기 공통) */
const BRAND_COLORS = {
  queen: {
    bg: "linear-gradient(160deg, #0E1B33 0%, #17294C 100%)",
    accent: "#BE9E5B",
    accentLight: "#D8C08A",
    label: "THE QUEEN",
  },
  table: {
    bg: "linear-gradient(160deg, #583047 0%, #6D3E55 100%)",
    accent: "#D4A574",
    accentLight: "#E8B088",
    label: "Queen's Table",
  },
